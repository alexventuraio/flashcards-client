...
class Main extends Component {
  constructor(props) {
    ...
    this.handleClickPreviousFlashcard = this.handleClickPreviousFlashcard.bind(
      this,
    );
    this.handleClickNextFlashcard = this.handleClickNextFlashcard.bind(this);
    this.handleCancelFlashcardDialog = this.handleCancelFlashcardDialog.bind(
      this,
    );
  ...
  handleClickPreviousFlashcard() {
    this.setState(prevState => {
      return {
        selectedFlashcardIndex: Math.max(
          0,
          prevState.selectedFlashcardIndex - 1,
        ),
      };
    });
  }
  ...
  handleClickNextFlashcard() {
    this.setState(prevState => {
      return {
        selectedFlashcardIndex: Math.min(
          prevState.selectedFlashcardIndex + 1,
  ...
  handleChangeFlashcard(field, value) {
    this.setState(prevState => {
      ...
      const newFlashcardInDialog = Object.assign({}, flashcardInDialog, {
        [field]: value,
      });
  ...
  fetchTags() {
    utils.fetchTags({token: this.props.token}).then(tags => {
      this.setState({tags});
    });
  }

  fetchFlashcards(tag) {
    utils.fetchFlashcards({token: this.props.token, tag}).then(flashcards => {
  ...
  createFlashcard(flashcard) {
    utils
      .createFlashcard({token: this.props.token, flashcard})
      .then(flashcard => {
  ...
  updateFlashcard(flashcard) {
    utils
      .updateFlashcard({token: this.props.token, flashcard})
      .then(flashcard => {
  ...
  deleteFlashcard(flashcard) {
    utils.deleteFlashcard({token: this.props.token, flashcard}).then(() => {
